...
pub async fn establish_connection(
    ...
    let cert = Certificate::from_pem(&ca_cert.as_bytes()).map_err(CustomErrorENUM::TlsError)?;
...
pub async fn fetch_anime_detail_by_al_id(
    client: &Client,
    anime_al_id: i32,
) -> Result<Anime, CustomErrorENUM> {
    let rows = client
        .query(Query::AnimeByAlId.sql(), &[&(anime_al_id)])
        .await
        .map_err(CustomErrorENUM::DatabaseError)?;
